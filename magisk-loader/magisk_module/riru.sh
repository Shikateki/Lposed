...
# Magisk 30.x exposes module files through its internal modules path.
# Keep the legacy path as a fallback for older Magisk versions.
RIRU_MODULE_PATH=""
if [ -d "$MAGISK_CURRENT_RIRU_MODULE_PATH" ]; then
  RIRU_MODULE_PATH="$MAGISK_CURRENT_RIRU_MODULE_PATH"
elif [ -d "/data/adb/modules/riru-core" ]; then
  RIRU_MODULE_PATH="/data/adb/modules/riru-core"
fi

check_riru_version() {
  if [ -z "$RIRU_MODULE_PATH" ] || { [ ! -f "$RIRU_MODULE_PATH/api_version" ] && [ ! -f "/data/adb/riru/api_version" ] && [ ! -f "/data/adb/riru/api_version.new" ]; }; then
    ui_print "*********************************************************"
  ...
  RIRU_API=$(cat "$RIRU_MODULE_PATH/api_version" 2>/dev/null) || RIRU_API=$(cat "/data/adb/riru/api_version.new" 2>/dev/null) || RIRU_API=$(cat "/data/adb/riru/api_version" 2>/dev/null) || RIRU_API=0
...
if [ -f "$RIRU_MODULE_PATH/disable" ] || [ -f "$RIRU_MODULE_PATH/remove" ]; then
  ui_print "*********************************************************"
...
if [ -f "$RIRU_MODULE_PATH/util_functions.sh" ]; then
  ui_print "- Load $RIRU_MODULE_PATH/util_functions.sh"
  . "$RIRU_MODULE_PATH/util_functions.sh"
elif [ "$RIRU_MODULE_MIN_API_VERSION" -ge 11 ]; then
  ui_print "*********************************************************"
  ui_print "! Riru $RIRU_MODULE_MIN_RIRU_VERSION_NAME or above is required"
  ui_print "! Please upgrade Riru from Magisk Manager or https://github.com/RikkaApps/Riru/releases"
  abort "*********************************************************"
